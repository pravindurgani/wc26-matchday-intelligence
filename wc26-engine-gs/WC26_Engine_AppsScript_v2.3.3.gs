/**
 * WC26 Value Betting Engine — Apps Script (v2.3.3)
 *
 * v2.3.3 (this file — adversarial-audit closure release on top of v2.3.2):
...
function installEngine() {
  ...
  SpreadsheetApp.getActive().toast(
    'WC26 Engine v2.3.3 installed. Auto-refresh every ' + POLL_MINUTES + ' min.',
    'Setup complete',
    8
  );
}
...
function refreshLive(state) {
  ...
  let staleMsg = '';
  // v2.3.3 LOW #14: also kill on circuit-breaker. live_state.warnings[]
  // may carry "circuit_breaker:*" entries from the upstream sim when its
  // provider-quota guard fires. Treat any such entry as a stale signal —
  // the model output we'd otherwise hand the Bets sheet is the LAST
  // known-good state from before the breaker tripped, which is exactly
  // the case staleKill exists to defend against.
...
function refreshOutrights(payload) {
  ...
  // v2.3.3 CRIT #2: preserve operator-typed bookie odds in col L across
  // re-sorts. The pre-fix clearContent(…1..18) wiped every L cell, so each
  // refreshOutrights (10-min tick) silently destroyed the operator's odds
  // and the BET? formula at col O reverted to "" until odds were re-typed.
  // Net: live BET? signals on the Outrights tab were unreachable in steady
  // state. Capture (team, L-value) before clear, then re-key onto the
  // post-sort row that holds the same team.
  const oddsByTeam = {};
  if (prevLast >= dataStart) {
    ...
    for (let i = 0; i < prevRange.length; i++) {
      const team = prevRange[i][0];           // col A
      const odds = prevRange[i][11];          // col L
...
function applyProtections() {
  ...
  Object.keys(editable).forEach(function(sheetName) {
    ...
    const p = sh.protect().setDescription('WC26 v2.3.3 — formulas locked');
...
function _writeCalibratedProbs_(bets, firstRow, rawProbs) {
  ...
  const out = rawProbs.map(function(row) {
    ...
    // Fallback to raw if any leg is non-numeric or the sum is degenerate.
...
function _writeStatus_(msg) {
  Logger.log(msg);
  try {
    SpreadsheetApp.getActive().toast(msg, 'WC26 Engine v2.3.3', 4);
  } catch (_) {}
}
