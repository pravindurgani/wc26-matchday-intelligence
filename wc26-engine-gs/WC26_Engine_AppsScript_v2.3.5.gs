/**
 * WC26 Value Betting Engine — Apps Script (v2.3.5)
 *
 * v2.3.5 (this file — closes seven regressions surfaced by adversarial
 *   pressure-test of v2.3.4. R32 kickoff is 2026-06-28; v2.3.4 carried a
 *   P0 blocker that would have left rows 74-105 (knockouts) unstakeable
 *   on the day even though extendToKnockouts() succeeded):
...
function installEngine() {
  ...
  SpreadsheetApp.getActive().toast(
    'WC26 Engine v2.3.5 installed. Auto-refresh every ' + POLL_MINUTES + ' min.',
    'Setup complete',
    8
  );
}
...
function _knockoutStakingFormulas_(r) {
  ...
  return { O_AQ: block_O_AQ, AR_AV: block_AR_AV, AZ_BA: block_AZ_BA };
}
...
function extendToKnockouts() {
  ...
  let msg = 'Extend to knockouts: added ' + added + ' new row(s), ' +
            ...
            ' AZ..BA=' + stakingSeeded_AZ_BA + ' row(s) seeded';
  if (conflicts) {
...
function applyProtections() {
  ...
  Object.keys(editable).forEach(function(sheetName) {
    ...
    const p = sh.protect().setDescription('WC26 v2.3.5 — formulas locked');
...
function _writeStatus_(msg) {
  Logger.log(msg);
  try {
    SpreadsheetApp.getActive().toast(msg, 'WC26 Engine v2.3.5', 4);
  } catch (_) {}
}
