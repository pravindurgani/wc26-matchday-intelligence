/**
 * WC26 Value Betting Engine — Apps Script (v2.3.7)
 *
 * v2.3.7 (this file — closes the FIVE operator-visibility / install-safety
...
function installEngine() {
  ...
  installAutoRefresh();
  applyProtections();
  // v2.3.7 HIGH-3: seed the walk-forward edge disclaimer next to the Kelly
  // fraction cell (Method!B5). Method!B73 carries walkForward2022Lift (set
  // by refreshDiagnostics → refreshAll above). Pull it, then write a
  // human-readable banner to C5 that flags the dis-alignment if any.
  _seedKellyEdgeDisclaimer_();
  SpreadsheetApp.getActive().toast(
    'WC26 Engine v2.3.7 installed (knockouts pre-seeded, sheets verified). ' +
...
function _preflightSheets_() {
  ...
  if (missing.length) {
    const msg = 'WC26 Engine install aborted — missing required sheet(s): ' +
                ...
                'tabs to match.';
    try { ss.toast(msg, 'WC26 Engine v2.3.7 install error', 30); } catch (_) {}
    throw new Error(msg);
  }
}

/**
 * v2.3.7 HIGH-3: surface a Kelly-fraction edge disclaimer beside
 ...
 * and writes a human-readable banner to Method!C5. Non-destructive: B5
 * itself is left alone; operator decides whether to adjust.
 *
 ...
 * back when the walk-forward edge looked closer to 5%. Suggest scaling
 * B5 ≈ 0.10 when lift ≤ 2%, leave 0.25 alone otherwise.
 */
function _seedKellyEdgeDisclaimer_() {
  ...
  if (!isFinite(lift)) {
    method.getRange('C5').setValue(
      'Walk-forward edge not yet read (Diagnostics pending). Default Kelly ' +
      'fraction B5=0.25 — re-run installEngine after refreshAll to populate.');
    return;
  }
  ...
  method.getRange('C5').setValue(banner);
}
...
/**
 * v2.3.7 MED-1: stamp Method!B82..B86 on each successful endpoint poll.
 * Operator can scan now() vs these cells to distinguish "endpoint down"
 * from "endpoint healthy but no match-edge in the current window".
 */
function _stampEndpoint_(cell) {
  try {
    ...
    const method = ss.getSheetByName(SHEET.method);
    if (!method) return;
    method.getRange(cell).setValue(new Date());
  } catch (_) {}
}
...
function refreshIntel(predictionsPayload) {
  ...
  try {
    ...
    // v2.3.7 HIGH-1: also stamp the persistent errLog so the failure
    // survives past the 4-sec status toast cycle.
    _setErrLog_('intel fetch failed: ' + e.message);
    return;
  }
...
function refreshDiagnostics() {
  ...
  try {
    ...
    _setErrLog_('walk_forward fetch failed: ' + e.message);
  }
  ...
  try {
    ...
    _setErrLog_('calibration fetch failed: ' + e.message);
  }
...
function _knockoutStakingFormulas_(r) {
  ...
  const block_O_AQ = [
    ...
    '=IF(S' + r + '="","",Method!$B$4*I' + r + '+(1-Method!$B$4)*S' + r + '/($S' + r + '+$T' + r + '+$U' + r + '))',
    '=IF(T' + r + '="","",Method!$B$4*J' + r + '+(1-Method!$B$4)*T' + r + '/($S' + r + '+$T' + r + '+$U' + r + '))',
    '=IF(U' + r + '="","",Method!$B$4*K' + r + '+(1-Method!$B$4)*U' + r + '/($S' + r + '+$T' + r + '+$U' + r + '))',
    '=IF(V' + r + '="","",V' + r + '*L' + r + '-1)',
    '=IF(W' + r + '="","",W' + r + '*M' + r + '-1)',
    '=IF(X' + r + '="","",X' + r + '*N' + r + '-1)',
    ...
    '=IF(AB' + r + '="","· enter odds",IF(AND(AF' + r + '>=Method!$B$7,AG' + r + '>=Method!$B$8),"BET","PASS"))',
  ...
  const block_AR_AV = [
    ...
    '=IF(AB' + r + '="","Type the 3 odds on the Matchday tab",IF(Method!$B$55=TRUE(),"🛑 PAUSED — drawdown "&TEXT(Method!$B$51,"0.0%")&" ≥ "&TEXT(Method!$B$52,"0.0%")&". Set Method!B54=Y to override.",IF(AH' + r + '="BET",IF(AJ' + r + '=0,"Edge found but stake rounds under £"&Method!$B$9&" — skip","Value found: EV "&TEXT(AF' + r + ',"0.0%")&", edge "&TEXT(AG' + r + ',"0.0%")),IF(AF' + r + '<Method!$B$7,"Price too short — EV "&TEXT(AF' + r + ',"0.0%")&" (need "&TEXT(Method!$B$7,"0.0%")&"+)","Model ≈ market — edge "&TEXT(AG' + r + ',"0.0%")&" (need "&TEXT(Method!$B$8,"0.0%")&"+)"))))',
...
function _matchdayKnockoutRowFormulas_(r) {
  ...
  return [
    ...
    '=IF(Bets!AH' + r + '="· enter odds","",IF(Method!$B$55=TRUE(),"🛑 PAUSED — drawdown protection. Override Method!B54=Y if intended.",IF(Bets!AH' + r + '<>"BET","❌ NO BET — price not good enough",IF(Bets!AJ' + r + '=0,"❌ NO BET — stake under minimum",IF(L' + rm + '="","🟡 WAIT — check team news first",IF(L' + rm + '="N","🔴 SKIP — team news concern",IF(L' + rm + '="Y","🟢 PLACE £"&Bets!AJ' + r + '&" on "&Bets!AD' + r + ',"")))))))',  // M: decision msg
...
function _writeStatus_(msg) {
  Logger.log(msg);
  try {
    SpreadsheetApp.getActive().toast(msg, 'WC26 Engine v2.3.7', 4);
  } catch (_) {}
}
