/**
 * WC26 Value Betting Engine — Apps Script (v2.3.1)
 *
 * v2.3.1 (this file — bug-fix release on top of v2.3):
...
function installEngine() {
  ...
  SpreadsheetApp.getActive().toast(
    'WC26 Engine v2.3.1 installed. Auto-refresh every ' + POLL_MINUTES + ' min.',
    'Setup complete',
    8
  );
}
...
function refreshIntel() {
  ...
  adjustments.forEach(function(a) {
    ...
    byMatch[mid].delta += _num_(a.total_elo_adjustment) || 0;
    const comps = Array.isArray(a.components) ? a.components : [];
    comps.forEach(function(c) {
      const v = _num_(c.capped_elo);
      if (!isFinite(v) || v === 0) return;
      const sign = v >= 0 ? '+' : '';
      byMatch[mid].parts.push(String(a.team) + ' ' + String(c.type) + ': ' + sign + v.toFixed(0));
    });
  });
...
function applyProtections() {
  ...
  Object.keys(editable).forEach(function(sheetName) {
    ...
    const p = sh.protect().setDescription('WC26 v2.3.1 — formulas locked');
...
function _writeStatus_(msg) {
  Logger.log(msg);
  try {
    SpreadsheetApp.getActive().toast(msg, 'WC26 Engine v2.3.1', 4);
  } catch (_) {}
}
