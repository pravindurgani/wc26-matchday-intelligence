/**
 * WC26 Value Betting Engine — Apps Script (v2.3.4)
 *
 * v2.3.4 (this file — closes six regressions found by independent re-audit
 *   of the v2.3.3 patches themselves. T-4d to R32 kickoff 2026-06-28):
...
function installEngine() {
  ...
  SpreadsheetApp.getActive().toast(
    'WC26 Engine v2.3.4 installed. Auto-refresh every ' + POLL_MINUTES + ' min.',
    'Setup complete',
    8
  );
}
...
function applyProtections() {
  ...
  Object.keys(editable).forEach(function(sheetName) {
    ...
    const p = sh.protect().setDescription('WC26 v2.3.4 — formulas locked');
...
function _writeCalibratedProbs_(bets, firstRow, rawProbs) {
  ...
  const out = rawProbs.map(function(row) {
    ...
    // Fallback to RAW probs (not '' strings) on any degeneracy. v2.3.4 HIGH #3.
    const allFinite = isFinite(nh) && isFinite(nd) && isFinite(na) &&
                      ch !== '' && cd !== '' && ca !== '';
    if (!allFinite) return [rh, rd, ra];
    const s = nh + nd + na;
    if (!(s > 0) || !isFinite(s)) return [rh, rd, ra];
    return [nh / s, nd / s, na / s];
  });
  bets.getRange(firstRow, BETS_COL.pHomeCorr, out.length, 3).setValues(out);
  // v2.3.4 HIGH #4: surface OOB-clamp counts on Live!intelWarn so the
  // operator notices when the draw calibrator's narrow range is biting.
  // Append rather than overwrite — refreshIntel may have populated this
  // cell already this tick.
  try {
    ...
    if (total > 0) {
      ...
      if (live) {
        ...
        const msg = '⚠ calibration OOB (clamp to last bin): ' +
                    'home=' + oobCounts.home + ' draw=' + oobCounts.draw +
                    ' away=' + oobCounts.away;
        live.getRange(LIVE_CELL.intelWarn).setValue(cur ? (cur + ' · ' + msg) : msg);
      }
    }
...
function _writeStatus_(msg) {
  Logger.log(msg);
  try {
    SpreadsheetApp.getActive().toast(msg, 'WC26 Engine v2.3.4', 4);
  } catch (_) {}
}
